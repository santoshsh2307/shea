    ...
    try {
      if (initialValues?.id) {
        ...
        if (photoFiles.length) {
          await uploadUserPhotos(initialValues.id, photoFiles.map((file) => file.originFileObj));
        }
        message.success("User updated successfully!");
      } else {
        // Create new user
        const response = await createUser(values, photoFiles.map((file) => file.originFileObj));
      ...
      <div style={{ padding: "12px" }}>
        <Card title={initialValues ? "Edit User" : "Create New User"}>
          ...
          >
            ...
            <Row gutter={16}>
              <Col span={24}>
                <Form.Item label="Profile Photos">
                  <Upload
                    ...
                    onRemove={(file) => setPhotoFiles(prev => prev.filter(item => item.uid !== file.uid))}
                  >
                    {photoFiles.length ? null : (
                      <div>
                        <UploadOutlined />
