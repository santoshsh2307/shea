...
import { Tabs, Table, Button, Modal, Form, Input, message, Carousel } from "antd";
...
export default function Services() {
  ...
  const [eventsData, setEventsData] = useState([
    { key: 1, event: "Maintenance", date: "2025-12-30" },
  ]);
  ...
  const handleOk = () => {
    form.validateFields().then(values => {
      switch (activeTab) {
        ...
        case "events":
          if (editingRecord) {
            setEventsData(prev => prev.map(r => (r.key === editingRecord.key ? { ...editingRecord, ...values } : r)));
            message.success("Event updated!");
          } else {
            setEventsData(prev => [...prev, { key: Date.now(), ...values }]);
