...
export default function Services() {
  ...
  const generatePDF = (record) => {
    ...
    if (record.pdf) {
      doc.text("PDF Attachment: Available", 20, 100);
    }
  ...
  const downloadUploadedPdf = (pdfFile) => {
    ...
    message.success("Uploaded PDF downloaded successfully!");
  };
      ...
      case "events":
        return [
          ...
          { 
            title: "PDF", 
            key: "pdf", 
            render: (_, record) => record.pdf ? <span>Uploaded</span> : <span>No PDF</span> 
          },
          {
            title: "Action",
            key: "action",
            render: (_, record) => (
              <>
                ...
                {record.pdf && <Button type="link" onClick={() => downloadUploadedPdf(record.pdf)}>Download Uploaded PDF</Button>}
                <Button type="link" danger onClick={() => handleDelete(record.key)}>Delete</Button>
              </>
            ),
          },
